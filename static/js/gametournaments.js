function postMatchHistorynew(oyuncu1, oyuncu2,name,score, player, ai) {
	$.ajax({
		url: '/updateMatchHistory/',
		type: 'POST',
		data: {
			'user1': oyuncu1,
			'user2': oyuncu2,
    ...
    postMatchHistoryTwo(oyuncu1, oyuncu2,name ,player, ai);
}

function postMatchHistoryTwo(oyuncu1, oyuncu2,name, player, ai) {
	$.ajax({
		url: '/addMatchHistory/',
		type: 'POST',
		data: {
...
function tournamentponggame(oyuncu1,oyuncu2,name)
{
    // Global Variables
var DIRECTION = {
	IDLE: 0,
...
var Game = {
	...
	endGameMenu: function (text) {
		...
		var winner = text == oyuncu1 ? oyuncu1 : oyuncu2;
    
        scorenew = this.player.score + " - " + this.ai.score;
		postMatchHistorynew(oyuncu1, oyuncu2 , name,scorenew, this.player.score, this.ai.score);
	...
	update: function () {
		if (!this.over) {
            ...
            // Move ai if they ai.move value was updated by the AI logic
            ...
            // If the ai collides with the bound limits, update the x and y coords.
		...
		else if (this.ai.score === 3) {
			this.over = true;
			setTimeout(function () {
				Pong.endGameMenu(oyuncu2);
