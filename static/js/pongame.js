function postMatchHistoryM(playerScore, aiScore,rakipAdi) {
	var user1 = localStorage.getItem('userName');
	var game = "TSK42PONG";
	var user2 = rakipAdi;
	$.ajax({
		url: '/postMatchHistory/',
		type: 'POST',
		data: {
			'user1': user1,
			'user2': user2,
			'user1score': playerScore,
			'user2score': aiScore,
			'game': game
		},
...
var Game = {
	...
	endGameMenu: function (text) {
		...
		var winner = text === "Winner!" ? "Player1" : rakipAdi;
    
		// Log the winner and the final score to the console

		postMatchHistoryM(Pong.player.score, Pong.ai.score,rakipAdi);
	...
	update: function () {
		...
		if (this.player.score == sayi) {
			...
			if (this.player.score == sayi) {
				this.over = true;
				setTimeout(function () {
					Pong.endGameMenu("Left Side Won!");
				}, 1000);
			} else {
		...
		// Check to see if the ai/AI has won the round.
			...
			this.over = true;
			setTimeout(function () {
				Pong.endGameMenu("Right Side Won!");
			}, 1000);
		}
	},
